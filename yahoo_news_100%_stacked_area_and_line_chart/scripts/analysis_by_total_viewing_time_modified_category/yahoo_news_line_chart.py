...
# データを加工
category_totals = df.groupby(['days', 'modified_category'])['viewing_time'].sum().unstack(fill_value=0).reindex(columns=category_list, fill_value=0)
...
ax.set_title('全ユーザー カテゴリー別の視聴時間')
ax.set_ylabel('視聴時間')
ax.set_xlabel('Days')
...
output_path = '../../data/img/analysis_by_total_viewing_time_modified_category/line_chart/all/line_chart_modified.png'
