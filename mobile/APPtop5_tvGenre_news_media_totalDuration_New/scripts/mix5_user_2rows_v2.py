...
for user in users:
    ...
    device_counts_1st = user_device_data_1st.groupby(['days', 'device_id']).size().unstack(fill_value=0).reindex(columns=[0, 1, 2, 3], fill_value=0).reset_index()

    # appデータからニュースカテゴリの使用時間合計を計算（1stデータセット）
    news_app_data_1st = user_app_data_1st[user_app_data_1st['app_category'] == 'ニュース']
    ...
    top_apps_duration_by_day_1st = user_app_data_1st.groupby(['days', 'app_name'])['duration'].sum().reset_index()
    ...
    category_duration_by_day_1st = user_media_data_1st[user_media_data_1st['media'] == 'TV'].groupby(['days', 'category'])['duration'].sum().unstack(fill_value=0).reset_index()
    # 全TVデータから全ジャンルを取得して固定順序リストを作成（1stデータセット）
    all_category_1st = user_media_data_1st[user_media_data_1st['media'] == 'TV']['category'].unique()
    all_category_1st.sort()
    ...
    category_duration_by_day_2nd = user_media_data_2nd[user_media_data_2nd['media'] == 'TV'].groupby(['days', 'category'])['duration'].sum().unstack(fill_value=0).reset_index()
    # 全TVデータから全ジャンルを取得して固定順序リストを作成（2ndデータセット）
    all_category_2nd = user_media_data_2nd[user_media_data_2nd['media'] == 'TV']['category'].unique()
    all_category_2nd.sort()
    ...
    for index, device_id in enumerate([0, 1, 2, 3]):
        ax.bar(device_counts_1st['days'] + index * bar_width, device_counts_1st[device_id], width=bar_width, color=device_colors[index], label=f'device_id {device_id}', align='edge')
    ax.set_title(f'user {user} device_id別 使用回数（1stデータセット ※データ無し）')
    ax.set_ylabel('使用回数')
    ...
    media_duration_by_type = user_media_data_1st[user_media_data_1st['media'].isin(media_types)].groupby(['days', 'media'])['duration'].sum().unstack(fill_value=0).reset_index()
    ...
    bar_width = 0.2
    for index, device_id in enumerate([0, 1, 2, 3]):
        ax.bar(device_counts_2nd['days'] + index * bar_width, device_counts_2nd[device_id], width=bar_width, color=device_colors[index], label=f'device_id {device_id}', align='edge')
    ...
    media_types = ['MB', 'PC']
    media_colors = sns.color_palette(palette_name, n_colors=len(media_types))
    media_duration_by_type = user_media_data_2nd[user_media_data_2nd['media'].isin(media_types)].groupby(['days', 'media'])['duration'].sum().unstack(fill_value=0).reset_index()
    if 'MB' not in media_duration_by_type.columns:
        media_duration_by_type['MB'] = 0
    if 'PC' not in media_duration_by_type.columns:
        media_duration_by_type['PC'] = 0
    ...
    output_path = f'../data/img/user/Img_mix5_{user}_2rows_v2.png'
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    plt.tight_layout()
    plt.savefig(output_path)
    plt.close()
    print(f'Img_mix5_{user}_2rows_v2.png was output.')