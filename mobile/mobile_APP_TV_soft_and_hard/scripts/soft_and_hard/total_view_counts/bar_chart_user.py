...
def plot_user_category_duration(df, file_name):
    ...
    for i, user in enumerate(users):
        ...
        category_totals.plot(kind='bar', stacked=True, color=sns.color_palette("hls", len(category_totals.columns)), ax=axes[i])
        axes[i].set_title(f'User {user}')
        axes[i].set_xlabel('Days')
        axes[i].set_ylabel('Total View Counts')

        # Set x-axis ticks and labels
        x_ticks = np.arange(len(days_range))  # Adjust x-ticks to start from 0 to align with the bars
        x_labels = [f'Day {day}' for day in days_range]
        axes[i].set_xticks(x_ticks)  # Set new x-ticks
    ...
    output_path = f'../../../data/img/soft_and_hard/view_counts/bar_chart/user/{file_name}'
    os.makedirs(os.path.dirname(output_path), exist_ok=True)

    # Save the figure
    plt.savefig(output_path, bbox_inches='tight')  # Add bbox_inches='tight' to fit the legend
    plt.close()
...
plot_user_category_duration(df_app, 'app_total_view_counts_user.png')
plot_user_category_duration(df_tv, 'tv_total_view_counts_user.png')
