...
def print_statistics(df, weekdays_col, period_col, duration_col):
    statistics_df = df.groupby([weekdays_col, period_col])[duration_col].agg(['mean', 'var', 'std']).reset_index()
    print(statistics_df)

def plot_boxplot(df, weekdays_col, period_col, duration_col, output_file, weekdays_results):
    ...
    for i, weekdays in enumerate(categories):
        ...
        sns.boxplot(x=weekdays_col, y=duration_col, hue=period_col, data=weekdays_df, ax=ax, palette="hls")
    ...
    plt.suptitle('Boxplot of Duration by weekdays and Experiment Period')
...
def plot_scatter(df, weekdays_col, period_col, duration_col, output_file, weekdays_results):
    categories = df[weekdays_col].unique()
    ...
    for i, weekdays in enumerate(categories):
        ...
        if cat_result:
            text_color = 'crimson' if cat_result[2] == "Significant" else 'black'
            ax.text(0.5, 0.90, f'p={cat_result[1]:.4f}, {cat_result[2]}', ha='center', transform=ax.transAxes, color=text_color, alpha=0.7, fontsize=18)
    plt.suptitle('Scatter Plot of Duration for Each weekdays by Period')
    for ax in axes:
        if ax.get_legend() is not None:
            ax.get_legend().remove()
...
def plot_violin(df, weekdays_col, period_col, duration_col, output_file, weekdays_results):
    categories = df[weekdays_col].unique()
    n_categories = len(categories)
    n_cols = min(n_categories, 3)
    n_rows = (n_categories + 2) // 3
    ...
    for i, weekdays in enumerate(categories):
        ax = axes[i]
        weekdays_df = df[df[weekdays_col] == weekdays]
        sns.violinplot(x=weekdays_col, y=duration_col, hue=period_col, data=weekdays_df, ax=ax, palette="hls", split=True)
        ...
        if cat_result:
            text_color = 'crimson' if cat_result[2] == "Significant" else 'black'
            ax.text(0.5, 0.90, f'p={cat_result[1]:.4f}, {cat_result[2]}', ha='center', transform=ax.transAxes, color=text_color, alpha=0.7, fontsize=18)
    plt.suptitle('Violin Plot of Duration by weekdays and Experiment Period')
    handles, labels = axes[0].get_legend_handles_labels()
    for ax in axes:
        if ax.get_legend() is not None:
            ax.get_legend().remove()
...
def plot_bar(df, weekdays_col, period_col, duration_col, output_file, weekdays_results):
    categories = df[weekdays_col].unique()
    n_categories = len(categories)
    n_cols = min(n_categories, 3)
    n_rows = (n_categories + 2) // 3
    ...
    for i, weekdays in enumerate(categories):
        ax = axes[i]
        weekdays_df = df[df[weekdays_col] == weekdays]
        sns.barplot(x=weekdays_col, y=duration_col, hue=period_col, data=weekdays_df, ax=ax, palette="hls", alpha=0.8, errorbar=None)
        ax.set_title(weekdays)
        ax.set_xlabel('')
        ...
        if cat_result:
            text_color = 'crimson' if cat_result[2] == "Significant" else 'black'
            ax.text(0.5, 0.90, f'p={cat_result[1]:.4f}, {cat_result[2]}', ha='center', transform=ax.transAxes, color=text_color, alpha=0.7, fontsize=18)
    plt.suptitle('Total Duration of Each weekdays for Each Period')
    handles, labels = axes[0].get_legend_handles_labels()
    for ax in axes:
        if ax.get_legend() is not None:
            ax.get_legend().remove()
...
def plot_histograms(df, weekdays_col, period_col, duration_col, output_file):
    categories = df[weekdays_col].unique()
...
input_folder = '../data/csv/complete1.5/'
output_folder_base = '../data/img/1.5/'
...
for file in os.listdir(input_folder):
    if file.endswith(".csv"):
        ...
        print("\n統計量（'weekdays', 'period'）：")
        print_statistics(combined_df, 'weekdays', 'period', 'duration')
        ...
        plot_bar(combined_df, 'weekdays', 'period', 'duration', f'{output_folder}/barplot.png', weekdays_results)
        plot_boxplot(combined_df, 'weekdays', 'period', 'duration', f'{output_folder}/boxplot.png', weekdays_results)
        plot_scatter(combined_df, 'weekdays', 'period', 'duration', f'{output_folder}/scatterplot.png', weekdays_results)
        plot_violin(combined_df, 'weekdays', 'period', 'duration', f'{output_folder}/violinplot.png', weekdays_results)
        plot_histograms(combined_df, 'weekdays', 'period', 'duration', f'{output_folder}/histogram.png')
        plt.close('all')
