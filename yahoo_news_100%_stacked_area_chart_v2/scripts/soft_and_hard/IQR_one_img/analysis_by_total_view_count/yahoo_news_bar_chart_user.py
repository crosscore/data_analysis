...
# Determine the number of users to plot and set up subplot dimensions
num_users = len(df['user'].unique())
rows = int(np.ceil(num_users / 3)) # Determine the number of rows needed for a 3-column layout

# Create a figure with subplots
fig, axes = plt.subplots(rows, 3, figsize=(15, 5*rows)) # Adjust the figsize as needed
