...
media_data = pd.read_csv('../data/csv/complete/Media.csv', dtype={'user': str})
device_data = pd.read_csv('../data/csv/complete/Device.csv', dtype={'user': str})
app_data = pd.read_csv('../data/csv/complete/APP.csv', dtype={'user': str})
...
axes[0].set_title('全ユーザー device_id別 使用回数')
...
axes[1].set_title('全ユーザー APP「ニュース」カテゴリ 累計視聴時間')
...
axes[2].set_title('全ユーザー APP TOP5+others 利用時間')
...
axes[3].set_title('全ユーザー Media (MB/PC) 使用時間')
...
axes[4].set_title('全ユーザー TVジャンル別 視聴時間')
...
output_path = '../data/img/all/Img_mix4_all_users.png'
os.makedirs(os.path.dirname(output_path), exist_ok=True)
plt.tight_layout()
plt.savefig(output_path)
plt.close()
print('Img_mix4_all_users.png was output.')
