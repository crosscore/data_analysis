...
def print_statistics(df, category_col, period_col, counts_col):
    statistics_df = df.groupby([category_col, period_col])[counts_col].agg(['mean', 'var', 'std']).reset_index()
    print(statistics_df)

def plot_boxplot(df, category_col, period_col, counts_col, output_file, category_results):
    ...
    for i, category in enumerate(categories):
        ...
        sns.boxplot(x=category_col, y=counts_col, hue=period_col, data=category_df, ax=ax, palette="hls")
    ...
    plt.suptitle('Boxplot of counts by Category and Experiment Period')
...
def plot_scatter(df, category_col, period_col, counts_col, output_file, category_results):
    categories = df[category_col].unique()
    ...
    for i, category in enumerate(categories):
        ...
        if cat_result:
            text_color = 'crimson' if cat_result[2] == "Significant" else 'black'
            ax.text(0.5, 0.90, f'p={cat_result[1]:.4f}, {cat_result[2]}', ha='center', transform=ax.transAxes, color=text_color, alpha=0.7, fontsize=18)
    plt.suptitle('Scatter Plot of counts for Each Category by Period')
    for ax in axes:
        if ax.get_legend() is not None:
            ax.get_legend().remove()
...
def plot_violin(df, category_col, period_col, counts_col, output_file, category_results):
    categories = df[category_col].unique()
    n_categories = len(categories)
    n_cols = min(n_categories, 3)
    n_rows = (n_categories + 2) // 3
    ...
    for i, category in enumerate(categories):
        ax = axes[i]
        category_df = df[df[category_col] == category]
        sns.violinplot(x=category_col, y=counts_col, hue=period_col, data=category_df, ax=ax, palette="hls", split=True)
        ...
        if cat_result:
            text_color = 'crimson' if cat_result[2] == "Significant" else 'black'
            ax.text(0.5, 0.90, f'p={cat_result[1]:.4f}, {cat_result[2]}', ha='center', transform=ax.transAxes, color=text_color, alpha=0.7, fontsize=18)
    plt.suptitle('Violin Plot of counts by Category and Experiment Period')
    handles, labels = axes[0].get_legend_handles_labels()
    for ax in axes:
        if ax.get_legend() is not None:
            ax.get_legend().remove()
...
def plot_bar(df, category_col, period_col, counts_col, output_file, category_results):
    categories = df[category_col].unique()
    n_categories = len(categories)
    n_cols = min(n_categories, 3)
    n_rows = (n_categories + 2) // 3
    ...
    for i, category in enumerate(categories):
        ax = axes[i]
        category_df = df[df[category_col] == category]
        sns.barplot(x=category_col, y=counts_col, hue=period_col, data=category_df, ax=ax, palette="hls", alpha=0.8, errorbar=None)
        ax.set_title(category)
        ax.set_xlabel('')
        ...
        if cat_result:
            text_color = 'crimson' if cat_result[2] == "Significant" else 'black'
            ax.text(0.5, 0.90, f'p={cat_result[1]:.4f}, {cat_result[2]}', ha='center', transform=ax.transAxes, color=text_color, alpha=0.7, fontsize=18)
    plt.suptitle('Total counts of Each Category for Each Period')
    handles, labels = axes[0].get_legend_handles_labels()
    for ax in axes:
        if ax.get_legend() is not None:
            ax.get_legend().remove()
...
def plot_histograms(df, category_col, period_col, counts_col, output_file):
    categories = df[category_col].unique()
...
for file in os.listdir(input_folder):
    if file.endswith(".csv"):
        ...
        print("\n統計量（'category', 'period'）：")
        print_statistics(combined_df, 'category', 'period', 'counts')
        ...
        plot_bar(combined_df, 'category', 'period', 'counts', f'{output_folder}/barplot.png', category_results)
        plot_boxplot(combined_df, 'category', 'period', 'counts', f'{output_folder}/boxplot.png', category_results)
        plot_scatter(combined_df, 'category', 'period', 'counts', f'{output_folder}/scatterplot.png', category_results)
        plot_violin(combined_df, 'category', 'period', 'counts', f'{output_folder}/violinplot.png', category_results)
        plot_histograms(combined_df, 'category', 'period', 'counts', f'{output_folder}/histogram.png')
        plt.close('all')
