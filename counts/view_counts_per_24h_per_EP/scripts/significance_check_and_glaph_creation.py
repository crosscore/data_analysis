...
def print_statistics(df, hour_col, period_col, counts_col):
    statistics_df = df.groupby([hour_col, period_col])[counts_col].agg(['mean', 'var', 'std']).reset_index()
    print(statistics_df)

def plot_boxplot(df, hour_col, period_col, counts_col, output_file, hour_results):
    categories = df[hour_col].unique()
    ...
    if n_rows == 1 and n_cols == 1:
        axes = np.array([axes])
    axes = axes.flatten()
    for i, hour in enumerate(categories):
        ax = axes[i]
        hour_df = df[df[hour_col] == hour]
        sns.boxplot(x=hour_col, y=counts_col, hue=period_col, data=hour_df, ax=ax, palette="hls")
        ax.set_title(hour)
        ax.set_xlabel('')
        ax.set_ylabel('counts' if i % n_cols == 0 else '')
        cat_result = next((item for item in hour_results if item[0] == hour), None)
        if cat_result:
    ...
    plt.suptitle('Boxplot of counts by hour and Experiment Period')
...
def plot_scatter(df, hour_col, period_col, counts_col, output_file, hour_results):
    categories = df[hour_col].unique()
    ...
    for i, hour in enumerate(categories):
        ...
        for period in periods:
            period_df = hour_df[hour_df[period_col] == period]
        ...
        if cat_result:
            text_color = 'crimson' if cat_result[2] == "Significant" else 'black'
            ax.text(0.5, 0.90, f'p={cat_result[1]:.4f}, {cat_result[2]}', ha='center', transform=ax.transAxes, color=text_color, alpha=0.7, fontsize=18)
    plt.suptitle('Scatter Plot of counts for Each hour by Period')
    for ax in axes:
        if ax.get_legend() is not None:
            ax.get_legend().remove()
...
def plot_violin(df, hour_col, period_col, counts_col, output_file, hour_results):
    categories = df[hour_col].unique()
    n_categories = len(categories)
    n_cols = min(n_categories, 3)
    n_rows = (n_categories + 2) // 3
    ...
    for i, hour in enumerate(categories):
        ax = axes[i]
        hour_df = df[df[hour_col] == hour]
        sns.violinplot(x=hour_col, y=counts_col, hue=period_col, data=hour_df, ax=ax, palette="hls", split=True)
        ...
        if cat_result:
            text_color = 'crimson' if cat_result[2] == "Significant" else 'black'
            ax.text(0.5, 0.90, f'p={cat_result[1]:.4f}, {cat_result[2]}', ha='center', transform=ax.transAxes, color=text_color, alpha=0.7, fontsize=18)
    plt.suptitle('Violin Plot of counts by hour and Experiment Period')
    handles, labels = axes[0].get_legend_handles_labels()
    for ax in axes:
        if ax.get_legend() is not None:
            ax.get_legend().remove()
...
def plot_bar(df, hour_col, period_col, counts_col, output_file, hour_results):
    categories = df[hour_col].unique()
    n_categories = len(categories)
    n_cols = min(n_categories, 3)
    n_rows = (n_categories + 2) // 3
    ...
    for i, hour in enumerate(categories):
        ax = axes[i]
        hour_df = df[df[hour_col] == hour]
        sns.barplot(x=hour_col, y=counts_col, hue=period_col, data=hour_df, ax=ax, palette="hls", alpha=0.8, errorbar=None)
        ax.set_title(hour)
        ax.set_xlabel('')
        ...
        if cat_result:
            text_color = 'crimson' if cat_result[2] == "Significant" else 'black'
            ax.text(0.5, 0.90, f'p={cat_result[1]:.4f}, {cat_result[2]}', ha='center', transform=ax.transAxes, color=text_color, alpha=0.7, fontsize=18)
    plt.suptitle('Total counts of Each hour for Each Period')
    handles, labels = axes[0].get_legend_handles_labels()
    for ax in axes:
        if ax.get_legend() is not None:
            ax.get_legend().remove()
...
def plot_histograms(df, hour_col, period_col, counts_col, output_file):
    categories = df[hour_col].unique()
    ...
    for i, hour in enumerate(categories):
        ...
        for j, period in enumerate(periods):
            ...
            subset = df[(df[hour_col] == hour) & (df[period_col] == period)]
...
for file in os.listdir(input_folder):
    if file.endswith(".csv"):
        ...
        print(combined_df)
        print(df_EP1['hour'].value_counts(dropna=False))
        print(df_EP2['hour'].value_counts(dropna=False))
        print(f"================================================")

        print("\n統計量（'hour', 'period'）：")
        print_statistics(combined_df, 'hour', 'period', 'counts')
        
        hour_results = []
        for hour in df['hour'].unique():
            ep1 = df_EP1[df_EP1['hour'] == hour]
            ep2 = df_EP2[df_EP2['hour'] == hour]
            print(f"\nhour: {hour}")
            result = compare_groups(ep1, ep2, 'counts', paired=True) #paired=True
            if result is not None:
                p_value, significance = result
                hour_results.append((hour, p_value, significance))
            else:
                print("Unable to perform statistical test because data is less than 3.")

        os.makedirs(output_folder, exist_ok=True)
        plot_bar(combined_df, 'hour', 'period', 'counts', f'{output_folder}/barplot.png', hour_results)
        plot_boxplot(combined_df, 'hour', 'period', 'counts', f'{output_folder}/boxplot.png', hour_results)
        plot_scatter(combined_df, 'hour', 'period', 'counts', f'{output_folder}/scatterplot.png', hour_results)
        plot_violin(combined_df, 'hour', 'period', 'counts', f'{output_folder}/violinplot.png', hour_results)
        plot_histograms(combined_df, 'hour', 'period', 'counts', f'{output_folder}/histogram.png')
        plt.close('all')
