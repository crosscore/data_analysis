...
for user in df['user'].unique():
    ...
    # データを加工
    category_totals = user_df.groupby(['days', 'modified_category'])['viewing_time'].sum().unstack(fill_value=0).reindex(columns=category_list, fill_value=0)
    ...
    ax.set_title(f'{user}のカテゴリー別視聴時間の割合')
    ax.set_ylabel('割合')
    ax.set_xlabel('Days')
    ...
    output_path = f'../../data/img/analysis_by_total_viewing_time_modified_category/100%_stacked_area_chart/user/{user}_stacked_area_chart_modified.png'
